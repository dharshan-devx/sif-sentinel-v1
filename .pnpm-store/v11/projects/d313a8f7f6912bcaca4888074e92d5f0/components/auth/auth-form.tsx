...
export function safeRedirectPath(value: string | null): string { return value?.startsWith("/") && !value.startsWith("//") ? value : "/dashboard"; }
function readableError(cause: unknown): string {
  if (!isApiClientError(cause)) return "We could not complete that request.";
  if (Array.isArray(cause.details)) {
    const messages = cause.details.flatMap((detail) => typeof detail === "object" && detail !== null && "msg" in detail && typeof detail.msg === "string" ? [detail.msg] : []);
...
export function AuthForm({ mode }: { mode: "login" | "register" }) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (mode === "login") { await signIn({ email, password }); router.replace(safeRedirectPath(params.get("next"))); }
  ...
  return <Card className="w-full max-w-md"><h1 className="text-2xl font-bold">{registerMode ? "Create an account" : "Sign in"}</h1><p className="mt-2 text-sm text-slate-600">{registerMode ? "New accounts have viewer access until provisioned by an administrator." : "Use your SIF Sentinel account to view safety signals."}</p>{error && <div id={errorId} className="mt-4"><Alert title={error} tone="danger" /></div>}<form className="mt-6 space-y-4" onSubmit={submit}>{registerMode && <label className="block text-sm font-semibold">Full name<Input className="mt-1" name="full_name" autoComplete="name" required maxLength={255} aria-invalid={Boolean(error)} aria-describedby={error ? errorId : undefined} /></label>}<label className="block text-sm font-semibold">Email<Input className="mt-1" name="email" type="email" autoComplete="email" required aria-invalid={Boolean(error)} aria-describedby={error ? errorId : undefined} /></label><label className="block text-sm font-semibold">Password<Input className="mt-1" name="password" type="password" autoComplete={registerMode ? "new-password" : "current-password"} minLength={registerMode ? 12 : 1} maxLength={128} required aria-invalid={Boolean(error)} aria-describedby={error ? errorId : undefined} /></label><Button className="w-full" type="submit" pending={pending}>{registerMode ? "Create account" : "Sign in"}</Button></form><p className="mt-5 text-center text-sm text-slate-600">{registerMode ? <>Already have an account? <Link className="font-bold text-sky-800 underline" href="/login">Sign in</Link></> : <>Need an account? <Link className="font-bold text-sky-800 underline" href="/register">Register</Link></>}</p></Card>;
}
